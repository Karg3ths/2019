...
public class Vyrazy {
    ...
    public static enum TokenType {
        NUMBER, VARIABLE, ASSIGNMENT, SUM, PRODUCT, EOF, PARSTART, PAREND
    }
    ...
    public static class Token {
        ...
        public static Token makeParStart() {
            return new Token(TokenType.PARSTART, 0, "");
        }
        public static Token makeParEnd() {
            return new Token(TokenType.PAREND, 0, "");
        }
        ...
        private void lexIt(String input) {
            ...
            for (char c : input.toCharArray()) {
                ...
                if (c == '+') {
                    ...
                    tokens.add(Token.makeParStart());
                } else if (c == ')'){
                    tokens.add(Token.makeParEnd());
                } else if (c == '#') {
    ...
    public static class Parser {
        ...
        private boolean nextTypesAre(TokenType... types) {//kontrola následujících tokenù
        ...
        private Node number() {
            ...
            } else if (nextTypesAre(TokenType.PARSTART)) {
                lexer.next();
                Node n = expression();
                if (lexer.next().type != TokenType.PAREND) {
                    String message = String.format("Expected %s, got %s.",
                        TokenType.PAREND, lexer.peek());
                    throw new IllegalArgumentException(message);
                }
                return n;
            } else {
